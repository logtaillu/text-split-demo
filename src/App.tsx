...
使用 React 开发时，需注意组件生命周期（类组件）或 Hook 调用规则，避免副作用导致的内存泄漏。虚拟 DOM  diff 算法采用深度优先遍历，通过 key 属性优化列表渲染效率。当前 React 最新版本持续优化并发渲染机制，提升应用响应性能。`);
    ...
    return (
        <div className='flex flex-col items-center justify-center gap-4 max-w-100 w-full m-auto'>
            <textarea
                onChange={(e) => { setText(e.target.value) }}
                className='w-full h-30 border border-solid rounded' placeholder='请输入文本'
                value={text}
            ></textarea>
            <div ref={source} className='w-full border border-solid rounded h-50 overflow-auto p-3 leading-10'></div>
            <div ref={target} className='w-full border border-solid rounded min-h-50 overflow-auto p-3'></div>
            <button onClick={splitText}>转换</button>
        </div>
    )
}
